...
class HonorController extends AdminbaseController{
	...
	function _initialize() {
		parent::_initialize();
		$this->slide_model = D("Common/Honor");
		$this->slidecat_model = D("Common/HonorCat");
	...
	function index(){
        ...
        $slidecount=$this->slide_model->where($where)->order("listorder ASC")->count();
        $page = $this->page($slidecount, 10);
        $this->assign('page', $page->show('Admin'));

        $slides=$this->slide_model->where($where)->order("listorder ASC")
		...
		if(IS_POST){
			if ($this->slide_model->create()) {
				...
				if ($this->slide_model->add()!==false) {
					$this->success("添加成功！", U("honor/index"));
				} else {
					$this->error("添加失败！");
				}
			} else {
		...
		if(IS_POST){
			if ($this->slide_model->create()) {
				...
				if ($this->slide_model->save()!==false) {
					$this->success("保存成功！", U("honor/index"));
				} else {
					$this->error("保存失败！");
				}
			} else {
